...
mydb=connection.MySQLConnection(user='root',host='localhost',password='_Praneeth_07_',db='ecom')
@app.route('/')
def home():
...
def additem():
    if session.get('admin'):
        if request.method=='POST':
            ...
            imgdata=request.files['file'] #'app.py' --['app','py']
            ...
            print(dirpath)
            static_path=os.path.join(dirpath,'static') #findign static path for app file
            print(static_path)
            imgdata.save(os.path.join(static_path,filename))
            try:
                cursor=mydb.cursor()
                cursor.execute('insert into items(itemid,itemname,description,quantity,cost,category,imagename,added_by) values(uuid_to_bin(uuid()),%s,%s,%s,%s,%s,%s,%s)',[item_name,description,quantity,price,item_category,filename,session.get('admin')])
                mydb.commit()
            except Exception as e:
                print(f'Error in mysql{e}')
                flash('could not save item data mysql server fail')
    ...
    if session.get('admin'):
        try:
            ...
            if imagename:
                ...
                static_path=os.path.join(dirpath,'static') #findign static path for app file
                print(static_path)
                os.remove(os.path.join(static_path,imagename[0]))
            ...
            print(e)
            flash('could not delete the items')
            return redirect(url_for('viewallitems'))
    else:
        flash('pls login first')
        return redirect(url_for('adminlogin'))
...
def usersignup():
    if request.method=='POST':
        ...
        address=request.form['address']
        otp=genotp()
        userdata={'username':username,'useremail':useremail,'password':password,'address':address,'otp':otp}
        subject=f'OTP for Ecom admincreate '
        body=f'use the otp {otp}'
        send_mail(to=useremail,body=body,subject=subject)
        flash(f'OTP has sent given mail {useremail}')
...
def userotp(endata):
    try:
        ...
        print(f'Error in dcode userdata {e}')
        flash('could not verify otp')
        ...
        if request.method=='POST':
            ...
            if uotp==ddata['otp']:
                ...
                try:
                    cursor=mydb.cursor(buffered=True)
                    cursor.execute('insert into userdata(username,useremail,password,address) values(%s,%s,%s,%s)',[ddata['username'],ddata['useremail'],hash,ddata['address']])
                    mydb.commit()
    ...
    if not session.get('user'):
        if request.method=='POST':
            ...
            try:
                ...
                email_count=cursor.fetchone()
                if email_count[0]==1:
                    cursor.execute('select password from userdata where useremail=%s',[useremail])
                    stored_password=cursor.fetchone()
                    if bcrypt.checkpw(password,stored_password[0]):
...
def viewcart():
    if session.get('user'):
        ...
        if items:
            ...
            flash('No items added to the cart')
            return redirect(url_for('index'))
    else:
        flash(f'pls login first')
        return redirect(url_for('userlogin'))
...
def addreview(itemid):
    if request.method=='POST':
        ...
        mydb.commit()
        cursor.close()
        flash('review added succesfully')
...
def userlogout():
    ...
    if session.get('user'):
        ...
        flash('to logout please login first')
        return redirect(url_for('userlogin'))
...
def searchdata():
    if request.method=='POST':
        ...
        strg=['A-Za-z0-9']
        MATCHING_STRING=re.compile(f'^{strg}',re.IGNORECASE)
        if MATCHING_STRING.search(sdata):
            try:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('select bin_to_uuid(itemid),itemname,description,quantity,cost,category,imagename from items where itemname like %s or cost like %s or category like %s',['%'+sdata+'%','%'+sdata+'%','%'+sdata+'%'])
                items=cursor.fetchall()
                cursor.close()
            except Exception as e:
                print(e)
...
def invoice(ordid):
    if session.get('user'):
        try:
            ...
            cursor.execute('select username,useremail,address from userdata where useremail=%s',[session.get('user')])
